...
// existirem. A integração "Postgres" da Vercel (via Neon) expõe a URL em
// DATABASE_URL ou POSTGRES_URL, dependendo de como foi criada.
let sqlClient;
...
function ensureSchema() {
  if (!schemaReady) {
    schemaReady = (async () => {
      ...
      `;
      await sql`
        INSERT INTO settings (id, site_name, phone, whatsapp, email, address, hours)
        VALUES (
          1, 'VCar Belém', '(91) 3222-1010', '5591988887777', 'contato@vcarbelem.com.br',
          'Av. Almirante Barroso, 2100 — Marco, Belém - PA', 'Segunda a sábado, 8h às 18h'
        )
...
export async function createCar(data) {
  await ensureSchema();
  const {
    name, brand, year, price, km = 0, transmission = 'Manual', fuel = 'Flex',
    color = '', photo_url = null, featured = false, description = ''
  } = data;
  const rows = await sql`
    INSERT INTO cars (name, brand, year, price, km, transmission, fuel, color, photo_url, featured, description)
    VALUES (${name}, ${brand}, ${year}, ${price}, ${km}, ${transmission}, ${fuel}, ${color}, ${photo_url}, ${featured}, ${description})
    RETURNING *;
  `;
  return rows[0];
}
